...
function! s:NosetestSyntax() abort
  let b:current_syntax = 'pytest'
  syn match NosetestPlatform              '\v^(platform(.*))'
  syn match NosetestTitleDecoration       "\v\={2,}"
  syn match NosetestTitle                 "\v\s+(test session starts)\s+"
  syn match NosetestCollecting            "\v(collecting\s+(.*))"
  syn match NosetestPythonFile            "\v((.*.py\s+))"
  syn match NosetestFooterFail            "\v\s+((.*)(failed|error) in(.*))\s+"
  syn match NosetestFooter                "\v\s+((.*)passed in(.*))\s+"
  syn match NosetestFailures              "\v\s+(FAILURES|ERRORS)\s+"
  syn match NosetestErrors                "\v^E\s+(.*)"
  syn match NosetestDelimiter             "\v_{3,}"
  syn match NosetestFailedTest            "\v_{3,}\s+(.*)\s+_{3,}"

  hi def link NosetestPythonFile          String
  hi def link NosetestPlatform            String
  hi def link NosetestCollecting          String
  hi def link NosetestTitleDecoration     Comment
  hi def link NosetestTitle               String
  hi def link NosetestFooterFail          String
  hi def link NosetestFooter              String
  hi def link NosetestFailures            Number
  hi def link NosetestErrors              Number
  hi def link NosetestDelimiter           Comment
  hi def link NosetestFailedTest          Comment
endfunction


function! s:NosetestFailsSyntax() abort
  let b:current_syntax = 'pytestFails'
  syn match NosetestQDelimiter            "\v\s+(\=\=\>\>)\s+"
  syn match NosetestQLine                 "Line:"
  syn match NosetestQPath                 "\v\s+(Path:)\s+"
  syn match NosetestQEnds                 "\v\s+(Ends On:)\s+"

  hi def link NosetestQDelimiter          Comment
  hi def link NosetestQLine               String
  hi def link NosetestQPath               String
  hi def link NosetestQEnds               String
endfunction
...
" Close the Nosetest buffer if it is the last one open
...
function! s:ClearAll(...)
    let current = winnr()
    let bufferL = [ 'Fails.pytest', 'LastSession.pytest', 'ShowError.pytest', 'NosetestVerbose.pytest' ]
    for b in bufferL
...
function! s:RunPyTest(path)
    ...
    for w in split(out, '\n')
        ...
        elseif w =~ '\v^(.*)\s*ERROR:\s+'
            call s:RedBar()
            echo "nosetests had an Error, see :Nosetest session for more information"
            if exists('$VIRTUAL_ENV')
              if !executable($VIRTUAL_ENV . "/bin/nosetests")
                echo repeat("*", 80)
                echo " Detected an activated virtualenv but nosetests was not found"
                echo " Make sure nosetests is installed in the current virtualenv"
                echo " and present at:"
                echo " "
                echo "    " . $VIRTUAL_ENV . "/bin/nosetests"
                echo " "
              endif
            endif
            return
        elseif w =~ '\v^(.*)\s*INTERNALERROR'
            call s:RedBar()
            echo "nosetests had an InternalError, see :Nosetest session for more information"
            return
        endif
    endfor
    call s:ParseSuccess(out)
...
function! s:ParseErrors(stdout)
    ...
    let error['error']     = get(error, 'error', 'nosetests had an error, please see :Nosetest session for more information')
    let errors[1] = error
...
function! s:ThisMethod(verbose, ...)
    ...
    let message = "nosetests ==> Running test for method " . m_name
    call s:Echo(message, 1)
    if len(a:2)
...
function! s:ThisFunction(verbose, ...)
    ...
    let message  = "nosetests ==> Running tests for function " . c_name
    call s:Echo(message, 1)
...
function! s:ThisClass(verbose, ...)
    ...
    let message  = "nosetests ==> Running tests for class " . c_name
    call s:Echo(message, 1)
...
function! s:ThisFile(verbose, ...)
    call s:ClearAll()
    let message = "nosetests ==> Running tests for entire file"
    call s:Echo(message, 1)
    let abspath = s:CurrentPath()
    if len(a:2)
...
function! s:ThisProject(verbose, ...)
    call s:ClearAll()
    let message = "nosetests ==> Running tests for entire project"
    call s:Echo(message, 1)
...
function! s:Pdb(path, ...)
    let pdb_command = "nosetests " . a:1 . " " . a:path
    if exists("g:ConqueTerm_Loaded")
...
function! s:Delgado(path, arguments, message)
    ...
    let json_arg = '{\"nosetests\" :'. args_as_list . '}'
...
function! s:Proxy(action, ...)
    ...
    if (executable("nosetests") == 0)
        call s:Echo("nosetests not found. This plugin needs nosetests installed and accessible")
        return
    endif
    ...
    elseif (a:action == "version")
        ...
        call s:Echo("Not a valid Nosetest option ==> " . a:action)
    endif
endfunction


command! -nargs=+ -complete=custom,s:Completion Nosetest call s:Proxy(<f-args>)
